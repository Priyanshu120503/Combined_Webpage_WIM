...
app.get("/register_home", (req, res) => {
    res.sendFile(__dirname + "/registration_home.html");
});
...
app.get("/analysis", async (req, res) => {
    ...
    users.forEach(user => {
        ...
        if(!cities.hasOwnProperty(user['city']))
            cities[user['city']] = [];    
        cities[user['city']].push(user['fName'] + " " + user['lName']);
    });
    
    let taken = 0;
    let location = {};
    Object.keys(cities).forEach(async city => {
        let loc = await fetch("https://api.api-ninjas.com/v1/geocoding?city=" + city, 
                                {method: 'GET', headers: {'X-Api-Key': process.env.API_NINJAS_API_KEY}}).
                                then(res => res.json()).
                                then(data => data) 
        
        if(loc.length > 0) { 
            location[JSON.stringify([loc[0].latitude, loc[0].longitude])] = cities[city];
            taken++;
        }
        else {
        ...
        if(taken == Object.keys(cities).length) {    
